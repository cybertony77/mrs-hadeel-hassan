...
export default function EditQuiz() {
  ...
  const { data: quizData, isLoading, error: quizError } = useQuery({
    ...
    queryFn: async () => {
      ...
      const quiz = response.data.quizzes.find(h => h._id === id);
  ...
  useEffect(() => {
    if (id && !isLoading) {
      if (quizError || (quizzesData && !quizzesData.quizzes.find(q => q._id === id))) {
        router.push('/dashboard/manage_online_system/quizzes');
      }
    }
  }, [id, quizError, isLoading, quizzesData, router]);

  // Load quiz data into form (only once when quizData first loads)
  ...
  const updateQuizMutation = useMutation({
    mutationFn: async (data) => {
      const response = await apiClient.put(`/api/quizzes?id=${id}`, data);
      return response.data;
    },
    onSuccess: () => {
    ...
    onError: (err) => {
      const errorMsg = err.response?.data?.error || 'Failed to update quiz';
  ...
  const handleSubmit = (e) => {
    ...
    console.log('Submitting quiz data:', {
      ...
      quiz_type: submitData.quiz_type,
      timer: submitData.timer,
      questions_count: submitData.questions?.length || 0,
      questions_with_images: submitData.questions?.filter(q => q.question_picture).length || 0
    });

    updateQuizMutation.mutate(submitData);
  };
      ...
      <div style={{ 
        ...
        <div style={{ maxWidth: 800, margin: "40px auto", padding: "20px 5px 20px 5px" }}>
          ...
          <div style={{
            ...
            <p style={{ color: "#6c757d", fontSize: "1rem" }}>Loading quiz...</p>
            <style jsx>{`
              @keyframes spin {
    ...
    return (
      <div style={{ 
        ...
        <div style={{ maxWidth: 800, margin: "40px auto", padding: "12px" }}>
          ...
          <div style={{ textAlign: 'center', padding: '40px', color: '#dc3545' }}>❌ Quiz not found</div>
        </div>
      </div>
    );
  }
                      ...
                      return (
                        <div key={aIdx} style={{ marginBottom: '12px' }}>
                          <div className="answer-option-row" style={{ 
                            ...
                            <input
                              type="text"
                              value={question.answer_texts && question.answer_texts[aIdx] ? question.answer_texts[aIdx] : ''}
                              onChange={(e) => {
                              ...
                              placeholder={`Enter text for option ${answerLetter} (optional)`}
                              style={{
                                flex: 1,
                                padding: '10px 14px',
                                border: '2px solid #e9ecef',
