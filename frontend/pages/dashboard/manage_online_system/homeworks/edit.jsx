...
export default function EditHomework() {
  ...
  // Fetch homework data
  const { data: homeworkData, isLoading, error: homeworkError } = useQuery({
    queryKey: ['homework', id],
    queryFn: async () => {
      const response = await apiClient.get('/api/homeworks');
      const homework = response.data.homeworks.find(h => h._id === id);
  ...
  useEffect(() => {
    if (id && !isLoading) {
      if (homeworkError || (homeworksData && !homeworksData.homeworks.find(h => h._id === id))) {
        router.push('/dashboard/manage_online_system/homeworks');
      }
    }
  }, [id, homeworkError, isLoading, homeworksData, router]);

  // Load homework data into form (only once when homeworkData first loads)
  ...
  }, [homeworkData, dataLoaded]);

  const updateHomeworkMutation = useMutation({
    mutationFn: async (data) => {
      const response = await apiClient.put(`/api/homeworks?id=${id}`, data);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries(['homeworks']);
      queryClient.invalidateQueries(['homework', id]);
      router.push('/dashboard/manage_online_system/homeworks');
    },
    onError: (err) => {
      const errorMsg = err.response?.data?.error || 'Failed to update homework';
  ...
  const handleSubmit = (e) => {
    ...
    console.log('Submitting homework data:', {
      lesson_name: submitData.lesson_name,
      homework_type: submitData.homework_type,
      ...
      ...(submitData.questions ? {
        ...
        questions: submitData.questions.map(q => ({
          has_picture: !!q.question_picture,
          picture_id: q.question_picture
        }))
      } : {
        book_name: submitData.book_name,
        from_page: submitData.from_page,
        to_page: submitData.to_page
      })
    });

    updateHomeworkMutation.mutate(submitData);
  };
